...
export default {
  ...
  stories: [
        ...
        },
        {
          ...
          verify: {
            ...
            custom: ({ stdout, stderr, exitCode }, input) => {
              ...
              if (/^(fix|update|change|edit|done|wip)$/i.test(message.trim())) return { ok: false, reason: 'be more specific — what exactly did you fix?' };
    ...
    // ─── Story 3: The Commit Message Archaeologist ────────────────────────────
    {
      ...
      setting: 'Something broke. It was working on Friday. It is not working now. Someone changed something over the weekend and the only way to figure out what is to read the commit history. You open it. The last six messages are: "fix", "actual fix", "real fix", "please work", "final", "final-final".',
      steps: [
        {
          id: 'read-the-history',
          narration: 'This is a real commit history. You\'ve seen it before — maybe you\'ve written it before.\n\n  `git log` shows every commit: who made it, when, and what they wrote. This is your archaeological dig through the project. The clues are whatever the developer left behind.\n\n  Sometimes the clues are useful. Sometimes the clues are "final-final." Either way, this is where you start.',
        ...
        },
        {
          ...
          verify: {
            ...
            custom: ({ stdout, stderr, exitCode }, input) => {
              ...
              if (!/git\s+show\s+[0-9a-f]{4,}/.test(input)) return { ok: false, reason: 'provide a commit hash after `git show` — copy one from `git log --oneline`' };
    ...
    // ─── Story 4: The README Disaster ────────────────────────────────────────
    {
      ...
      setting: 'You deleted README.md. You didn\'t mean to — you were cleaning up the root directory and your muscle memory betrayed you. Nothing crashed. The tests still pass. Nobody has noticed yet. This somehow makes everything worse.',
      steps: [
        {
          id: 'confirm-the-crime',
          narration: 'The first thing to do is confirm what git knows.\n\n  When you delete a tracked file, git sees it immediately — it shows up in `git status` as "deleted." This is actually good news: if git is tracking it, git can restore it.\n\n  Check the status. See it for yourself.',
